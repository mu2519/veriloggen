...
from veriloggen.stream.pump import PumpMult
from veriloggen.stream import stypes
...
def mkdut():
    ...
    a_list = strm.Split(a_packed, in_datawidth * 2, signed=False)
    b_list = strm.Split(b_packed, in_datawidth * 2, signed=False)
    prod_list = []
    for a, b in zip(a_list, b_list):
        prod_list += stypes.Split(PumpMult(a, b, clk2x), num_chunks=2, signed=True)
...
def mktb():
    ...
    m.Initial(
        clk(0),
        Forever(clk(~clk, ldelay=10))
    )
    m.Initial(
        clk2x(0),
        Delay(5),
        Forever(clk2x(~clk2x, ldelay=5))
    )
